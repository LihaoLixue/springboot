...
CREATE OR REPLACE PROCEDURE cust.p_ids_xy_init_cacl(
                            i_rq in int
)is
/*********************************************************************************************
...
DECLARE 
    ...
    l_tableName_sparkJjjzHis STRING;--基金净值
    l_tableName_sparkTfjjjxx STRING;--分级基金信息
...
BEGIN
    ...
    --证券行情
    BEGIN
        l_tableName_sparkZqhqHisToday :=l_dbname||'sparkZqhqHisToday';
    ...
    --证券类别
    BEGIN
        l_tableName_sparkZqlb :=l_dbname||'sparkZqlb';
    ...
    --获取基金净值
    BEGIN
        l_tableName_sparkJjjzHis :=l_dbname||'xy_sparkJjjzHis'||l_suffix;
        l_sqlBuf:="select tadm,jjdm,jjjz,ljjz,mwfsy,nsyl,jzrq,yhsyl,sylpcjd,gsjz,fhps,isgsjz,rq from DSC_BAS.T_FJJJJZ_HIS  where jzrq="||i_rq;
                    
        F_IDS_CREATE_TEMP_TABLE(l_sqlBuf,l_tableName_sparkJjjzHis);
    END;
    
    --分级基金信息
    BEGIN
        l_tableName_sparkTfjjjxx :=l_dbname||'xy_sparkTfjjjxx'||l_suffix;
        l_sqlBuf:="select * FROM INFO.TFJJJXX"
                    
        F_IDS_CREATE_TEMP_TABLE(l_sqlBuf,l_tableName_sparkTfjjjxx);
    END;    
    

    
    BEGIN
        l_tableName_sparkFzxxbdmx := l_dbname||'xy_sparkJgmxlsXYQS'||l_suffix;
    ...
    BEGIN
        l_tableName_sparkFzxx := l_dbname||'xy_sparkFzxx'||l_suffix;
    ...
    BEGIN
        l_tableName_sparkZqyeGT:=l_dbname||'xy_sparkZqyeGT_0'||l_suffix;
        ...
        END;
        BEGIN
            l_tableName_sparkZqyeGTResult:=l_dbname||'xy_sparkZqyeGTResult'||l_suffix;
            l_sqlBuf := "select
                   0 as tzzh_id,
                   khh,
                   jys,
        ...
        --成本
        BEGIN
            l_tableName_sparkZqyeCbjsGT :=l_dbname||'xy_sparkZqyeCbjsGT'||l_suffix;
        ...
        BEGIN
            ...
            --1、生成资金余额数据
            BEGIN
                l_tableName_sparkzcZjye :=l_dbname||'xy_sparkzcZjye'||l_suffix;
            ...
            --2、生成证券市值数据
            BEGIN
                l_tableName_sparkzcZqsz :=l_dbname||'xy_sparkzcZqsz'||l_suffix;
                l_sqlBuf:="SELECT  tzzh_id,
                    KHH,
                    0 AS zzc_sr       ,
                    0 AS zzc         ,
            ...
            --3、融资融券负债数据            
            BEGIN
                l_tableName_sparkzcFzxx :=l_dbname||'xy_sparkzcFzxx'||l_suffix;
                l_sqlBuf:="SELECT  TZZH_ID,
                    KHH,
                    0 AS zzc_sr       ,
                    0 AS zzc         ,
                    ...
                    0 AS zrsz         ,
                    0 AS zcsz         ,
                    CAST(ROUND(SUM((RZFZ+RQFZ+YJLX+FXYJLX) * CASE BZ WHEN '2' THEN " || l_hlcsHKD || " WHEN '3' THEN " || l_hlcsUSD || " ELSE 1 END),2) AS DECIMAL(16,2)) AS ZFZ,
                    0 AS zfz_sr       ,
                    0 AS cccb         ,
                    0 AS dryk         ,
                    0 AS tzzh_fe     ,
             ...
             FROM(SELECT 0 AS TZZH_ID,
                        ...
                        END AS RQFZ,                    
                                        JYLB,
                        CASE WHEN FZZT=0 THEN YJLX ELSE 0 END AS YJLX,
                        GHLX,
                        HKJE,
                        KHH,
                        FXYJLX
            ...
            --4、处理在途资产等修正数据
            BEGIN
                l_tableName_sparkzcZcxz :=l_dbname||'xy_sparkzcZcxz'||l_suffix;
                l_sqlBuf:= "SELECT 0 AS TZZH_ID,
                       KHH,
                        0 AS zzc_sr,
                        0 AS zzc,
            ...
            BEGIN
                l_tableName_sparkzcFZxxbdmx :=l_dbname||'xy_sparkzcFZxxbdmx'||l_suffix;
                l_sqlBuf:= "SELECT  0 AS TZZH_ID,
                    D.KHH,
                    0 AS zzc_sr       ,
                    0 AS zzc         ,
            ...
            BEGIN
                l_tableName_sparkStatDR :=l_dbname||'xy_sparkStatDR'||l_suffix;
                l_sqlBuf:="select * from "||l_tableName_sparkzcZjye||"
                            ...
                            UNION ALL
                            select 0 AS TZZH_ID,KHH,zzc_sr,zzc,zqsz,zjye,ztzc,qtzc,crje,qcje,zrsz,zcsz,ZFZ,zfz_sr,cccb,dryk,tzzh_fe,tzzh_fe_sr,tzzh_zxfe,tzzh_zxfe_sr,tzzh_zxjz,tzzh_zxjz_sr,tzzh_ljjz,tzzh_ljjz_sr,0 as RZFZ,0 as RQFZ,0 as YJLX,0 as GHLX,0 as yjlx_sr,lxsr,lxzc_qt,jyl,0 as xzrqfz,0 as xzrqyjlx,0 as xzrzyjlx,0 as xzfz,0 as xzhkje,0 as xzrqhkje,0 as xzrzhkje,0 as mqhqje,0 as xzghlx,0 as xzrzghlx,0 as xzrqghlx,0 as HKJE,0 as mqhkje  from "||l_tableName_sparkzcZcxz;
                            
                F_IDS_CREATE_TEMP_TABLE(l_sqlBuf,l_tableName_sparkStatDR);
            END;
            
            BEGIN
                l_tableName_sparkZctjResult :=l_dbname||'xy_sparkZctjResult'||l_suffix;
                l_sqlBuf:="SELECT  TZZH_ID,
                    KHH,
                  ...
                  (SELECT TZZH_ID,
                        ...
                        ELSE
                          IF(SUM(TZZH_ZXFE_SR) is NULL,
                                    ...
                                    (nvl(SUM(CRJE),0) + nvl(SUM(LXSR),0) - nvl(SUM(QCJE),0) - nvl(SUM(LXZC_QT), 0) + nvl(SUM(ZRSZ),0) - nvl(SUM(ZCSZ),0)) / nvl(SUM(TZZH_LJJZ_SR), 1.0))
                      END AS TZZH_FE,
                      ...
                      CASE
                        ...
                        ELSE
                          IF(SUM(TZZH_ZXFE_SR) is NULL,
                                    ...
                                    (nvl(SUM(CRJE),0) + nvl(SUM(LXSR),0) - nvl(SUM(QCJE),0)- nvl(SUM(LXZC_QT), 0) + nvl(SUM(ZRSZ),0) - nvl(SUM(ZCSZ),0)) / nvl(SUM(TZZH_ZXJZ_SR), 1.0))
                      END AS TZZH_ZXFE,
                ...
                F_IDS_OVERWRITE_PARTITION(l_tableName_sparkZctjResult,"cust","t_stat_xy_zc_r",i_rq,NULL);
            END;
        END;
 END;
 /