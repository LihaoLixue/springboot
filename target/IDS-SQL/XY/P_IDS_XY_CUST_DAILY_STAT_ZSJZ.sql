...
CREATE OR REPLACE PROCEDURE cust.p_ids_xy_cust_daily_stat_zxjz(
--输入变量
I_RQ IN INT,
I_KHH IN STRING
)
  ...
  BEGIN
    ...
    l_sqlBuf := "SELECT TZZH_ID,
     ...
     F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName);
	   ...
	   -- 净值折算
  BEGIN
	l_tableName := F_IDS_GET_TABLENAME('xy_sparkZsjzResult', I_KHH);	
	l_sqlBuf :="select  DR.TZZH_ID,
						...
						DR.rq
				from "||F_IDS_GET_TABLENAME('xy_sparkZctjResult', I_KHH)||" dr
				left join (SELECT   ConvertWorth(z.khh, 
									...
									nvl(s.tzzh_ljjz, 1.0), 
									nvl(s.cache,''))
						   FROM  "||F_IDS_GET_TABLENAME('xy_sparkZctjResult', I_KHH)||" z
						   LEFT JOIN "|| F_IDS_GET_TABLENAME('xy_sparkZcSR', I_KHH) ||"  s 
								  on z.khh = s.khh) zs 
					   on dr.khh = zs.khh ";
	F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName);
