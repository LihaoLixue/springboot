...
CREATE OR REPLACE PROCEDURE cust.p_ids_daily_bill_njzq_jzzs(
                            i_rq in int,
                            i_khh in STRING ,
							i_is_init in int default 0
)is
/*********************************************************************************************
 ...
 -- 数据插入至资产统计临时表sparkStatDR
  ...
  ----------------------------------------/sparkRzd_2-----------------------------------------------------
    ...
    l_sqlBuf :=     "select  t.khh,"||
                    ...
                    " zzc-zfz     "||
                    " else  "||
                    " srfe +  ((crje+zrzqsz+qtsr) - (qcje+zczqsz+qtzc-nvl(jd.zcsz_dryk,0)-nvl(xd.zcsz_dryk,0)))/nvl(srjz,1.0)"||
                    " end as zxfe,"||
                    " zzc_jzjy,"||
                    " zqsz_jzjy,"||
                    " zjye_jzjy,"||
  ...
  ----------------------------------------/sparkRzdResult-----------------------------------------------------
    ...
    l_sqlBuf := "select KHH,"||
            ...
            "     END " ||
			"	WHEN ZZC = ZJYE AND ((CRJE != 0 AND CRJE = QCJE) OR ZCJLR!=0) THEN YK/(SRZZC - SRZFZ + CRJE + ZRZQSZ)" || 
			"   WHEN ZCJLR < 0 THEN ROUND(((ZZC - ZFZ)-ZCJLR)/(SRZZC - SRZFZ) - 1.0,6) " || 
	        "   WHEN ZCJLR > 0 THEN  " ||
            "        CASE WHEN (SRZZC - SRZFZ + ZCJLR) = 0 THEN 0 ELSE 
            ROUND((ZZC-ZFZ)/((SRZZC-SRZFZ)+ZCJLR)-1.0, 6) END " ||
   ...
   F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName)
    ...
    ----------------------------------------/进行净值折算-----------------------------------------------------
		l_tableName := F_IDS_GET_TABLENAME('sparkRzdResult_zsjz', I_KHH);
		l_sqlBuf :="select dr.khh, 
                           dr.zzc, 
                           dr.zjye, 
                           dr.zqsz, 
                           dr.zfz, 
                           ...
                           DR.zxjz AS ZXJZ_OLD,
						   dr.rq
                     from "||  F_IDS_GET_TABLENAME('sparkRzdResult', I_KHH) ||" dr
                     left join (SELECT ConvertWorth(z.khh,
                                                    ...
                                                    NVL(z.yk,0.0),
                                                    NVL(z.qtsr,0.0),";
		-- 初始化时不能关联上日的数据，避免误跑初始化前的数据时，导致初始化计算异常
		-- I_IS_INIT 默认传参为0，初始化时添加传参 1 
		IF I_IS_INIT=1 THEN 
			l_sqlBuf:= l_sqlBuf || "				0.0, 
													0.0,  
													1.0, 
													'')  
								FROM  "||  F_IDS_GET_TABLENAME('sparkRzdResult', I_KHH) ||" z 
								LEFT JOIN "||  F_IDS_GET_TABLENAME('sparkSrzd', I_KHH) ||" s  
									   on z.khh = s.khh) zs 
							on dr.khh = zs.khh" ;
		else l_sqlBuf:= l_sqlBuf || "				NVL(s.zxfe,0.0),
													NVL(s.zxjz,0.0),
													NVL(s.ljjz, 1.0),
													NVL(s.cache,''))   
								FROM  "||  F_IDS_GET_TABLENAME('sparkRzdResult', I_KHH) ||" z 
