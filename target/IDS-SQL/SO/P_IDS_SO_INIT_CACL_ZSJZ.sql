...
CREATE OR REPLACE procedure cust.p_ids_so_init_cacl_zsjz(
  --输入变量
  I_RQ IN INT
...
/******************************************************************
 ...
 燕居庆      v1.0.3     2019/11/15           增加其他收入、其他支出
  -----------------------------------------------------------------------
 邱建锋		 v2.0.0		20210105			调整为折算净值版本，表结构需调整
...
BEGIN
 ...
 BEGIN
   ...
   */ 
        -- 净值折算
  BEGIN
	l_tableName := F_IDS_GET_TABLENAME('so_sparkZsjzResult', NULL);	
	l_sqlBuf :="select  DR.KHH,
						...
						DR.rq
				from "|| F_IDS_GET_TABLENAME('so_sparkZctjResult', null) ||" dr
				left join (SELECT   ConvertWorth(z.khh, 
						   ...
						   FROM  "||F_IDS_GET_TABLENAME('so_sparkZctjResult', NULL) ||" z ) zs 
					   on dr.khh = zs.khh ";
	F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName);
