...
create or replace procedure cust.p_ids_so_cust_daily_stat_zsjz(
  --输入变量
  I_RQ IN INT,
  I_KHH IN STRING
...
---------------------------------------- //5、生成当日统计最终数据------------------------------------------------
  ...
  BEGIN
    ...
    l_sqlBuf := "SELECT   " ||
             ...
             "                      (NVL(SUM(CRJE),0) + NVL(SUM(LXSR),0) - NVL(SUM(LXZC), 0) + NVL(SUM(QTSR), 0) - NVL(SUM(QTZC_FY), 0) - NVL(SUM(QCJE),0) + NVL(SUM(ZRSZ),0) - NVL(SUM(ZCSZ),0)) / NVL(SUM(TZZH_LJJZ_SR), 1.0))   " ||
             "        END AS TZZH_FE,   " ||
             ...
             "                      (NVL(SUM(CRJE),0) + NVL(SUM(LXSR),0) - NVL(SUM(LXZC), 0) + NVL(SUM(QTSR), 0) - NVL(SUM(QTZC_FY), 0) - NVL(SUM(QCJE),0) + NVL(SUM(ZRSZ),0) - NVL(SUM(ZCSZ),0)) / NVL(SUM(TZZH_ZXJZ_SR), 1.0))   " ||
             "        END AS TZZH_ZXFE,   " ||
             ...
             "    GROUP BY  KHH) T LEFT JOIN CUST.T_KHXX_JJYW K   " ||
             "   ON k.KHH = T.KHH 
			      WHERE k.khrq <=  " || I_RQ || "
				        and (k.khzt != '3' or (k.khzt = '3' and k.xhrq >= " || I_RQ || " )))A";
  ...
  BEGIN
	...
	l_sqlBuf :="select  DR.KHH,
						...
						DR.rq
				from "|| F_IDS_GET_TABLENAME('so_sparkZctjResult', I_KHH) ||" dr
				left join (SELECT   ConvertWorth(z.khh, 
									...
									nvl(s.tzzh_ljjz, 1.0), 
									nvl(s.cache,'')) 
						   FROM  "||F_IDS_GET_TABLENAME('so_sparkZctjResult', I_KHH) ||" z
						   LEFT JOIN "|| F_IDS_GET_TABLENAME('so_sparkZcSr', I_KHH) ||"  s 
								  on z.khh = s.khh) zs 
					   on dr.khh = zs.khh ";
	F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName);
