...
create or replace procedure cust.p_ids_finance_product_cost_njzq(
  --输入变量
  I_RQ IN INT,
  I_KHH IN STRING
...
/******************************************************************
  ...
  *文件说明：集中交易-金融产品成本盈亏计算
  ...
  燕居庆        v1.0.1        2019/8/19       金融产品份额改为柜台获取sparkFPcpfe
  燕居庆        v1.0.2        2019/9/16       对标java-ids的4858版本：
  燕居庆        v1.0.3        2019/9/24       针对南证现金宝A72001特殊处理：cpsl=0不计入清仓，计入持仓计算累计盈亏                
*******************************************************************/
...
BEGIN
  ...
  BEGIN
     ...
     * 集中：F_IDS_GET_TABLENAME('sparkFPCbjsPre1', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPCbjsPre1', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPCbjsPre1', I_KHH);
     */
    l_tableName := F_IDS_GET_TABLENAME('sparkFPCbjsPre1', I_KHH);
  ...
  BEGIN
     ...
     * 临时表名
     * 由于集中/两融/期权可能存在相同临时表，因此在改造时，需要添加相关前缀：
     * 集中：F_IDS_GET_TABLENAME('sparkFPcpfe', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPcpfe', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPcpfe', I_KHH);
     */
    ...
    l_sqlBuf := " SELECT T.KHH,T.JRCPZH,T.JRJGDM,T.CPDM,CPJC,CAST('1' AS STRING) SFFS,SUM(T.CPSL) AS CPSL,T.BZ,SUM(T.ZXSZ) AS ZXSZ," ||
                ...
                " FROM DSC_BAS.T_FP_CPFE_HIS T WHERE (T.CPSL > 0 OR T.CPDM = 'A72001') AND RQ = " || l_sqlWhereCurrentDay ||
  ...
  BEGIN
     ...
     * 临时表名
     * 由于集中/两融/期权可能存在相同临时表，因此在改造时，需要添加相关前缀：
     * 集中：F_IDS_GET_TABLENAME('sparkFPjgmxls', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPjgmxls', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPjgmxls', I_KHH);
     */
    l_tableName := F_IDS_GET_TABLENAME('sparkFPjgmxls', I_KHH);
  ...
  BEGIN
     ...
     * 临时表名
     * 由于集中/两融/期权可能存在相同临时表，因此在改造时，需要添加相关前缀：
     * 集中：F_IDS_GET_TABLENAME('sparkFPcpdm', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPcpdm', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPcpdm', I_KHH);
     */
    l_tableName := F_IDS_GET_TABLENAME('sparkFPcpdm', I_KHH);
  ...
  --------------------------获取金融产品份额--------------------------------
     ...
     * 临时表名
     * 由于集中/两融/期权可能存在相同临时表，因此在改造时，需要添加相关前缀：
     * 集中：F_IDS_GET_TABLENAME('sparkFPcpfePre1', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPcpfePre1', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPcpfePre1', I_KHH);
     */
    l_tableName := F_IDS_GET_TABLENAME('sparkFPcpfePre1', I_KHH);
...
-------------------------------------   //1、生成当日买入、当日卖出相关数据---------------------------------  
  BEGIN
    l_tableName := F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_1', I_KHH);
    l_sqlBuf :=                 
                ...
                "               CPDM," ||
                "               BZ," ||
                "               SUM(CASE WHEN YWDM IN ('122', '123', '139', '130', '143', '144') THEN ABS(QRFE)" ||
                "                        WHEN YWDM IN ('124', '125', '142', '143', '145', '150') THEN -ABS(QRFE)" ||
                "                        ELSE 0 " ||
                "                   END) AS YSSL," ||
                "               SUM(CASE" ||
                "                       WHEN YWDM IN ('122', '123', '139', '130', '144') THEN" ||
                "                        ABS(QRJE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                   END) AS MRJE," ||
                "               SUM(CASE" ||
                "                       WHEN YWDM IN ('122', '123', '139', '130', '143', '144') THEN" ||
                "                        ABS(QRFE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                   END) AS MRSL," ||
                "               SUM(CASE" ||
                "                       WHEN YWDM IN ('124', '125', '142', '143', '145', '150') THEN" ||
                "                        ABS(QRJE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                   END) AS MCJE," ||
                "               SUM(CASE" ||
                "                       WHEN YWDM IN ('124', '125', '142', '145', '150') THEN" ||
                "                        ABS(QRFE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                   END) AS MCSL," ||
                "               SUM(SXF) AS JYFY" ||
                "          FROM " || F_IDS_GET_TABLENAME('sparkFPjgmxls', I_KHH) || "  " ||
                "         WHERE YWDM IN ('122', '123', '124', '125', '139', '142', '130', '143', '144', '145', '150')" ||
...
-------------------- //2、将本日数据和上日累计数据进行累计处理------------------------------------
  BEGIN
    l_tableName := F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_2', I_KHH);
    l_sqlBuf :=
                "SELECT 0 AS TZZH_ID," ||
                "       D.KHH," ||
                "       D.JRJGDM," ||
                ...
                "          FROM " || F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_1', I_KHH) || "  " ||
                ...
                "          FROM " || F_IDS_GET_TABLENAME('sparkFPCbjsPre1', I_KHH) || "  ) D" ||
 ...
 -----------------------------------    //3、获取当日的份额市值以及上日的成本盈亏等数据-----------------------------
  BEGIN
    l_tableName := F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_3', I_KHH);
    l_sqlBuf := 
                "SELECT A.TZZH_ID," || 
                  I_RQ || " AS RQ," ||
                "       A.KHH," ||
                ...
                "  FROM " || F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_2', I_KHH) || "  A" ||
                "  LEFT JOIN (SELECT JRJGDM, CPDM, JRCPZH, CPSL, ZXSZ, KHH, APP_ID" ||
                "               FROM " || F_IDS_GET_TABLENAME('sparkFPcpfe', I_KHH) || ") B" ||
                ...
                "               FROM " || F_IDS_GET_TABLENAME('sparkFPcpfePre1', I_KHH) || ") C" ||
 ...
 ----------------------------------  //4、生成最终数据------------------------------------------ 
   BEGIN
    l_tableName := F_IDS_GET_TABLENAME('sparkFPCbjsResult', I_KHH);
    l_sqlBuf := 
                "SELECT T1.TZZH_ID," ||
                "           T1.KHH," ||
                "           T1.JRCPZH," ||
                ...
                "                        0" ||
                "                   END AS KCRQ" ||
                "              FROM  " || F_IDS_GET_TABLENAME('sparkFPCpfeCbjs_3', I_KHH) || " T" ||
                "              LEFT JOIN " || F_IDS_GET_TABLENAME('sparkFPcpdm', I_KHH) || " D" ||
                "                ON T.JRJGDM = D.JRJGDM" ||
                "               AND T.CPDM = D.CPDM" ||
                "               AND T.APP_ID = D.APP_ID" ||
                "       /*WHERE NOT(T.CPSL = 0" ||
                "         OR (T.DRMCSL = 0 AND (T.SRCPSL=0)))*/) T1" ||
                "      LEFT JOIN (SELECT A.TZZH_ID, A.JRCPZH, A.JRJGDM, A.CPDM, A.KCRQ, A.KHH, A.APP_ID" ||
                "                   FROM " || F_IDS_GET_TABLENAME('sparkFPCbjsPre1', I_KHH) || " A" ||
                "                  WHERE A.CPSL > 0" ||
                "                    AND A.KCRQ <> 0) T2" ||
...
----------------------------回写T_FP_CPFE_HIS------------------------------------------ 
  BEGIN
     ...
     * 临时表名
     * 由于集中/两融/期权可能存在相同临时表，因此在改造时，需要添加相关前缀：
     * 集中：F_IDS_GET_TABLENAME('sparkFPcpfeResult', I_KHH);
     * 两融：F_IDS_GET_TABLENAME('xy_sparkFPcpfeResult', I_KHH);
     * 期权：F_IDS_GET_TABLENAME('so_sparkFPcpfeResult', I_KHH);
     */
    l_tableName := F_IDS_GET_TABLENAME('sparkFPcpfeResult', I_KHH);
    l_sqlBuf :=  
                "SELECT T.KHH," ||
                "       T.JRCPZH," ||
                "       T.JRJGDM," ||
                ...
                "               APP_ID," ||
                I_RQ || " as              rq" ||
                "          FROM  " || F_IDS_GET_TABLENAME('sparkFPcpfe', I_KHH) || " ) T" ||
                ...
                "               FROM  " || F_IDS_GET_TABLENAME('sparkFPCbjsResult', I_KHH) || " ) D" ||
                "    ON T.KHH = D.KHH" ||
                "   AND T.JRJGDM = D.JRJGDM" ||
                "   AND T.CPDM = D.CPDM" ||
                "   AND T.JRCPZH = D.JRCPZH" ||
                "   AND T.APP_ID = D.APP_ID" ||
                "   AND (T.CPSL != 0 OR T.CPDM <> 'A72001') ";
    /**
     * 调用创建临时表函数创建临时表，入参：建表的select语句，临时表名称
     */
    F_IDS_CREATE_TEMP_TABLE(l_sqlBuf, l_tableName);
  ...
  BEGIN
     ...
     * F_IDS_OVERWRITE_PARTITION(tablename, dbname,targetTable, partitionCloumn, partitionValue, khh)
     */ 
    F_IDS_OVERWRITE_PARTITION(l_tableName, "CUST", "T_FP_CPFE_HIS", "RQ", I_RQ, I_KHH);
  END; 
...
----------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    l_sqlBuf := 
                ...
                "                   T.DRMRJE," ||
                "                   T.DRMCJE" ||
                "              FROM " || F_IDS_GET_TABLENAME('sparkFPCbjsResult', I_KHH) || " T) D" ||
                "      LEFT JOIN " || F_IDS_GET_TABLENAME('sparkFPcpdm', I_KHH) || " T" ||
                "        ON T.JRJGDM = D.JRJGDM" ||
                "       AND T.CPDM = D.CPDM" ||
                "       AND T.APP_ID = D.APP_ID" ||
                "     WHERE D.CPSL = 0" ||
