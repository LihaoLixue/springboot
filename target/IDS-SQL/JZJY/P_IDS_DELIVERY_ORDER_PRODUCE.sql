...
/******************************************************************
  ...
  *文件说明：集中交易-交割单清算后处理
  ...
  邱建锋		v1.0.6		  2021/01/21	  港股证券代码变更，关联上日原代码持仓获取成本
  钟梦涵                v1.0.7            20211021 中国能建吸收葛洲坝，葛洲坝提前转出，特殊处理转移成本
*******************************************************************/
...
BEGIN
  ...
  BEGIN
    ...
    l_sqlBuf := "SELECT  T.LSH,T.TZZH_ID,T.SEQNO,T.WTH,T.KHH,T.KHXM,T.GDH,T.JYS,T.BZ,T.YYB,T.ZQDM,T.ZQMC,T.ZQLB,T.JYLB," ||
				...
				"	   WHEN T.JYLB = '95' AND T.JYS IN ('8','9') AND BG.BGDM IS NOT NULL THEN CONCAT(BG.ZQDM,'内部代码变更二') "|| -- 20210121 针对A2港股合并做修正
				"	   WHEN T.JYLB = '19' AND T.YSSL<0 AND BGQ.BGDM IS NOT NULL THEN '内部代码变更' "||
				"	   WHEN T.JYLB = '18' AND T.YSSL>0 AND BG.BGDM IS NOT NULL THEN CONCAT(BG.ZQDM,'内部代码变更') "||   -- 20200528 证券代码变更成本修正
                ...
                "  FROM " || F_IDS_GET_TABLENAME('sparkJgmxlsHis', I_KHH) ||"  T
				  ...
				  LEFT JOIN INFO.TZQDMBG BG ON (BG.BGLB = 3 AND T.CJRQ = BG.BGRQ AND T.ZQDM = BG.BGDM AND T.JYS = BG.JYS)  "||     --20210121
				"  LEFT JOIN INFO.TZQDMBG BGQ ON (BGQ.BGLB = 3 AND T.CJRQ = BGQ.BGRQ AND T.ZQDM = BGQ.ZQDM AND T.JYS = BGQ.JYS)
                  LEFT JOIN (SELECT
                    DISTINCT ZQDM
                              , GPDM,sgrq,
                  ...
                  FROM (SELECT T.TZZH_ID,
                               ...
                               (CASE
                                   ...
                                   WHEN T.JYS = '2' AND D.JJLX = '1' THEN
                                    T.CJSL
                                   WHEN T.JYS = '1' AND D.JJLX <> '1' THEN
                                    T.WTSL * CAST(D.ABBL AS DECIMAL) / 10           ON (J.KHH = A.KHH AND J.TZZH_ID = A.TZZH_ID AND J.GDH = A.GDH AND J.JYS = A.JYS AND
 ...
 --修正后的持仓成本,顺带更新盲拆的清算序列号
  BEGIN
    ...
    l_sqlBuf := "SELECT T.TZZH_ID,
                       ...
                       T.ZQMC,
                       T.ZQLB,
                       CASE WHEN HG.KHH IS NOT NULL THEN '18' ELSE T.JYLB END AS JYLB,
                       T.CJBH,
                       T.CJSJ,
                       T.SBSJ,
                       T.CJBS,
                       ...
                       T.CJJE,
                       T.LXJE,
                       CASE WHEN B.KHH IS NOT NULL THEN 0-B.CCCB
					        WHEN HG.KHH IS NOT NULL THEN 0-HG.CCCB 
                       ...
                       T.JJJZ,
                       (CASE
                           WHEN D.KHH IS NOT NULL AND T.YSSL != 0 AND T.JYLB = '42' THEN
                            1
                           ELSE
                            T.SEQNO_QS
                       END) AS SEQNO_QS,
				  ...
				  LEFT JOIN " || F_IDS_GET_TABLENAME('sparkZqyeSR', I_KHH) || " HG	"||-- 20210121 港股证券代码变更，关联上日原代码持仓获取成本
                 "   ON (T.KHH = HG.KHH AND T.GDH = HG.GDH AND T.JYS = HG.JYS 
				   AND HG.JYS IN ('8','9')
				   AND T.CJBH LIKE '%内部代码变更二%' 
				   AND SUBSTRING(T.CJBH,0,5) = HG.ZQDM)
                  LEFT JOIN " || F_IDS_GET_TABLENAME('sparkZqyeSR', I_KHH) || " B
                       ON T.KHH=B.KHH AND T.CJBH LIKE '%内部代码变更' AND T.ZQDM='601868' AND T.JYS='2' AND T.CJRQ=20210927 AND B.ZQDM='600068'"; --20210927 中国能建吸收葛洲坝，葛洲坝无转出记录，特殊处理转移成本
