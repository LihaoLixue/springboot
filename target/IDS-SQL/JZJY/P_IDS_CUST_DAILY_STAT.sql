 ...
 ------------------------------------//2、生成证券市值数据----------------------------------------------------
  BEGIN
    l_sqlBuf := 
                ...
                "        cast(0 AS decimal(16,2)) AS hlxz,        " ||
                "        cast(0 AS decimal(16,2)) AS zcsz_yk      " ||
                "  FROM (select * from CUST.T_ZQYE_HIS WHERE RQ = " || l_sqlWhereCurrentDay || ") T LEFT SEMI JOIN ( select * from " ||
                " dsc_cfg.vw_t_zqlb_ids where IS_JSSZ = 1) D ON ( T.ZQLB =D.ZQLB AND D.JYS = T.JYS)" ||
                " GROUP BY TZZH_ID, KHH";    
...
-------------------------------------//4、处理在途资产等修正数据------------------------------------------------------
  BEGIN
    l_sqlBuf := 
                ...
                "        cast(0 AS decimal(16,2)) AS hlxz,        " ||
                "        cast(0 AS decimal(16,2)) AS zcsz_yk      " ||
                "  FROM  DSC_STAT.T_STAT_KHZCXZMX T WHERE ZHLB IN ('1', '3') and "|| I_RQ ||" between ksrq and jsrq "|| l_khh ||
                "  GROUP BY KHH";
...
-------------------------------------//5、生成存入资金、取出资金数据------------------------------------------
  BEGIN
    l_sqlBuf := "SELECT 0 AS TZZH_ID," ||
                ...
                "       CAST(SUM(CASE WHEN SJKM = '102' and ywkm not in ('13014') " ||
                "                    THEN fsje * CASE BZ " ||
                "                                WHEN '2' THEN " || l_hlcsHKD ||
                "                                WHEN '3' THEN " || l_hlcsUSD ||
                "                                ELSE" ||
                ...
                "                               END" ||
                "                    ELSE 0" ||
                "                END) AS DECIMAL(16,2)) AS LXZC_QT," ||
                "        0 AS jyl,          " ||
                "        cast(0 AS decimal(16,2)) AS hlxz,        " ||
                "        cast(0 AS decimal(16,2)) AS zcsz_yk      " ||
 ...
 ------------------------------------//10、股票质押利息支出：初始交易贷款到的金额需要扣除费用，即扣除费用后所贷到的金额才是真正后续资金使用额-------------------  
  BEGIN
    l_sqlBuf := "SELECT 0 AS TZZH_ID," ||
                ...
                "         CAST(SUM(CASE WHEN CJJE != 0 THEN CJJE - ABS(YSJE) ELSE 0 END) AS DECIMAL(16,2)) AS JYFY_GPZY," ||
                "        cast(0 AS decimal(16,2)) AS lxzc_qt,     " ||
                "        cast(0 AS decimal(16,2)) AS jyl,         " ||
                "        cast(0 AS decimal(16,2)) AS hlxz,        " ||
                "        cast(0 AS decimal(16,2)) AS zcsz_yk      " ||
...
-------------------------------------//11、上日数据---------------------------------------------- 
  ...
  BEGIN
    ...
    l_sqlBuf := "SELECT 0 AS TZZH_ID," ||
                ...
                "      (SELECT KHH,BZ,ZQDM,SUM(ZQSL) ZQSL,SUM(LJMRJE) LJMRJE,SUM(LJMCJE) LJMCJE FROM " || F_IDS_GET_TABLENAME('sparkZqyeCbjs', I_KHH) ||" GROUP BY KHH,BZ,ZQDM) C" ||
                ...
                "      (SELECT KHH,BZ,CPDM,SUM(CPSL) ZQSL,SUM(LJMRJE) LJMRJE,SUM(LJMCJE) LJMCJE FROM "|| F_IDS_GET_TABLENAME('sparkFpCbjs', I_KHH) || " GROUP BY KHH,BZ,CPDM) C" ||
